...
class Node:
    ...
    def __eq__(self, other):
        if not isinstance(other, Node):
            return False
        return self.ip == other.ip and self.port == other.port and self.id == other.id
    ...
    def process_requests(self, message):
        ...
        if operation == "notify":
            # print("notifiying")
            self.notify(int(args[0]),args[1],args[2],bool(int(args[3])))
    ...
    def handle_delete_server(self):
    	...
    	if self.successor.id == self.id and self.predecessor.id == self.id:
    	    print("This is the last node in the ring. Deleting and shutting down.")
    	    self.close_connection()
    	    return
    	print(self.predecessor.port)
    	if self.predecessor:
    	    self.request_handler.send_message(
		    self.predecessor.ip, 
		    self.predecessor.port, 
		    f"notify|{self.successor.id}|{self.successor.ip}|{self.successor.port}|1"
	    	)
    	...
    	if self.successor:
		    ...
		    self.successor.ip, 
		    self.successor.port, 
		    f"notify|{self.predecessor.id}|{self.predecessor.ip}|{self.predecessor.port}|0"
	    	)
	    	
    	if self.data_store.data and self.successor:
    	    for key, value in self.data_store.data.items():
    	    	self.request_handler.send_message(
		    self.successor.ip, 
		    self.successor.port, 
		    f"insert_server|{key}:{value}"
	    	)
        
    	self.data_store.data.clear()
    	self.predecessor = None
    	self.successor = None
    	print(f"Node {self.id} deleted and ring updated.")
    	self.close_connection()
    ...
    def close_connection(self):
    	try:
    		print(f"Closing connections for Node {self.id}...")
    ...
    def start(self):
        ...
        thread_for_monitor_exit = threading.Thread(target=self.monitor_exit, daemon=True)
        thread_for_monitor_exit.start()
        
        self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.server_socket.bind((self.nodeinfo.ip, self.nodeinfo.port))
    ...
    def monitor_exit(self):
     	while not self.shutdown_event.is_set():
             user_input = input("Type 'exit' to delete the server and exit: ").strip()
    ...
    def stabilize(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            if self.successor.ip == self.ip  and self.successor.port == self.port:
            	#print("hey hello how r u  %%%%%%%% @@@@@@@@ ####################################################")
            	time.sleep(10)
            result = self.request_handler.send_message(self.successor.ip , self.successor.port , data)
            if result == "None" or len(result) == 0:
                self.request_handler.send_message(self.successor.ip , self.successor.port, "notify|"+ str(self.id) + "|" + self.nodeinfo.__str__() + "|0")
                continue
            ...
            if self.predecessor.ip == self.ip  and self.predecessor.port == self.port:
            	#print("hey hello how r u  %%%%%%%% @@@@@@@@ ####################################################")
            	time.sleep(10)
            result = self.request_handler.send_message(self.predecessor.ip , self.predecessor.port , data)
            if result == "None" or len(result) == 0:
                self.request_handler.send_message(self.predecessor.ip , self.predecessor.port, "notify|"+ str(self.id) + "|" + self.nodeinfo.__str__() + "|1")
                continue

            # print("found successor of my predecessor", result, self.predecessor.id)
            ip , port = self.get_ip_port(result)
            result = int(self.request_handler.send_message(ip,port,"get_id"))
            if self.get_forward_distance(result) < self.get_forward_distance(self.predecessor.id):
                # print("changing my pred in stablaize", result)
                self.predecessor = Node(ip,port)
                self.finger_table.table[0][1] = self.predecessor
            self.request_handler.send_message(self.predecessor.ip , self.predecessor.port, "notify|"+ str(self.id) + "|" + self.nodeinfo.__str__() + "|1")
    ...
    def notify(self, node_id, node_ip, node_port, is_successor):
        '''
        Handles notifications from neighboring nodes to update the predecessor or successor.

        :param node_id: ID of the notifying node
        :param node_ip: IP of the notifying node
        :param node_port: Port of the notifying node
        :param is_successor: Boolean to indicate if this is a successor notification
        '''
        updated = False  # Flag to indicate whether an update was made

        # Update predecessor logic
        if not is_successor:  # Notification is about the predecessor
            new_predecessor = Node(node_ip, int(node_port))
            if self.predecessor != new_predecessor:
                print(f"Updating predecessor to node {node_id} ({node_ip}:{node_port})")
                self.predecessor = new_predecessor
                updated = True

        # Update successor logic
        if is_successor:  # Notification is about the successor
            new_successor = Node(node_ip, int(node_port))
            if self.successor != new_successor:
                print(f"Updating successor to node {node_id} ({node_ip}:{node_port})")
                self.successor = new_successor
                self.finger_table.table[0][1] = new_successor  # Update finger table with new successor
                updated = True

        # Prevent further notifications if no updates were made
        if not updated:
            #print("No updates required. Skipping further notifications.")
            return

        # Notify the new predecessor or successor if updates were made
        if not is_successor and self.predecessor:
            self.request_handler.send_message(
                self.predecessor.ip,
                self.predecessor.port,
                f"notify|{self.id}|{self.ip}|{self.port}|1"
            )
        elif is_successor and self.successor:
            self.request_handler.send_message(
                self.successor.ip,
                self.successor.port,
                f"notify|{self.id}|{self.ip}|{self.port}|0"
            )
...
class RequestHandler:
    ...
    def send_message(self, ip, port, message):
    	if not is_socket_alive(ip, port):
    		print(f"Socket at {ip}:{port} is not alive. Message not sent.")
    		return "None"
    	...
    	try:
    		...
    		print(f"Error sending message to {ip}:{port}. Error: {e}")
    		return f"Error: {e}"
    	finally:
    		s.close()
...
def is_socket_alive(ip, port):
    """Check if a socket is alive by attempting to connect."""
    try:
